...
class FederatedDataOrchestrator:
    ...
    def _load_data_flexible(self, path, dataset_type):
        # Absoluter Pfad-Check für AWS
        abs_path = os.path.abspath(path)
        if not os.path.exists(abs_path):
            print(f"❌ DATEI NICHT GEFUNDEN: {abs_path}")
            # Wir werfen hier einen Fehler, damit die Simulation nicht blind weiterläuft
            raise FileNotFoundError(f"Datensatz für {dataset_type} fehlt unter {abs_path}")

        if abs_path.endswith(".npz"):
            data = np.load(abs_path)
            x = data['x'].reshape(len(data['x']), -1)
            y = data['y'].squeeze()
            # Erstellt den DataFrame und stellt sicher, dass die Spalte 'label' heißt
    ...
    def build(self, rsna_root, chexpert_root, verbose=False):
        ...
        self.pca_pool = []  # RAM freigeben
        self.raw_pools = {}  # RAM freigeben
        cleanup_memory()  # Meine Funktion in der utils.py
