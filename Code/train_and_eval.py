...
def evaluate_model(model, data_loader, device, minority_idx=None):
    ...
    return {
        ...
        # PER-CLASS METRICS (Lückenlos für Klasse 0 und 1)
        'precision_c0': prec[0],  # Normal
        'recall_c0': rec[0],  # Specificity
        'f1_score_c0': f1[0],

        'precision_c1': prec[1],  # Pneumonia
        'recall_c1': rec[1],  # Sensitivity
        'f1_score_c1': f1[1],

        # LDS / MINORITY ANALYSIS (Dynamisch)
        'f1_minority': f1[minority_idx],
        'recall_minority': rec[minority_idx],
        'precision_minority': prec[minority_idx],
        'f1_majority': f1[majority_idx],

        # BIAS & FAIRNESS
        'performance_gap': performance_gap,  # |F1_c0 - F1_c1|
